...
#delete column
    ...
    def selection(self, percent=25):
        print " --> selection "
        k = int(self.whh.shape[0] * percent / 100)
    ...
    def cross_over(self, percent=65):
        print "-->cross-over"
        k = int(self.whh.shape[0] * percent / 100)
        ind = sorted(range(self.whh.shape[0]), key=lambda i: self.scores[i])[:k]
        for i in ind:
            self.insert_in_model(i)
        print "There is now {} neurons ".format(self.whh.shape[0])

    def noise(self, vector):
        #return vector
        cov = 0.01 * np.diag(vector)
        res = np.random.multivariate_normal(vector, cov)
        return res.astype(theano.config.floatX)

    def insert_in_model(self, i):
        #insert in whh
        self.whh = np.array(np.hstack([self.whh, np.matrix(self.noise(self.whh[:, i])).T]))  # TODO noise
